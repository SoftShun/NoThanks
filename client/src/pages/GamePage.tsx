...
const GamePage: React.FC = () => {
  ...
  const [cardDisplayMode, setCardDisplayMode] = useState<'stack' | 'numbers'>('numbers');
          ...
          <div className="panel">
            ...
            <div className="row two" style={{ marginTop: 8 }}>
              ...
              <div className="field">
                ...
                <select id="showTokens" className="input" value={showOpponentTokens ? 'public' : 'private'} onChange={(e)=> setShowOpponentTokens(e.target.value === 'public')}>
              ...
              <div className="field">
                <label className="field-label" htmlFor="cardMode">카드 표시 방식</label>
                <select id="cardMode" className="input" value={cardDisplayMode} onChange={(e)=> setCardDisplayMode(e.target.value as any)}>
                  <option value="stack">작은 카드 겹치기</option>
                  <option value="numbers">숫자 나열(연속 묶음 박스)</option>
                </select>
              </div>
            </div>
            <div className="controls" style={{ marginTop: 8 }}>
              <button
                ...
                onClick={async () => {
                  if (starting) return;
                  setStarting(true);
                  const ok = await startGame({ removedCount: removedCount || 1, initialTokens: initialTokens || 1, showOpponentTokens, cardDisplayMode });
                  setStarting(false);
                  if (!ok) return;
                }}
                aria-label="게임 시작"
                  ...
                  {p.cards.length > 0 && (
                    <div className="my-cards" aria-label={isYou ? '내 카드' : `${p.nickname}님의 카드`}>
                      {(() => {
                        ...
                        if (state.cardDisplayMode === 'numbers') {
                          return (
                            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                              {groups.map((g, gi)=> (
                                <div key={`g-${gi}`} style={{ display: 'flex', gap: 6, padding: 6, border: '1px dashed var(--border)', borderRadius: 8 }}>
                        ...
                        groups.forEach((g, gi) => {
                          g.forEach((c, idx) => {
                            const left = offset + idx * 24;
                            elements.push(
                              <div key={`c-${gi}-${idx}`} className="card-small appear-pop" style={{ left }} aria-label={`카드 ${c}`}>
                                {c}
                              </div>
                            );
                          });
                          // 다음 그룹 시작 간격 추가
                          offset += (g.length - 1) * 24 + 28;
                        });
                        return elements;
                      })()}
                    </div>
